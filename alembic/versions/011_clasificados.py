"""corte global clasificados + recalc excluyendo retiradas

Revision ID: 011_clasificados
Revises: 010_criterio_reglamento
Create Date: 2026-09-22
"""
from alembic import op

revision = "011_clasificados"
down_revision = "010_criterio_reglamento"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    op.execute("""
-- ============================================================
-- MIGRACION 011 — Clasificación: excluir retiradas + corte global
-- Ejecutar en Supabase SQL Editor (producción). Idempotente.
-- ============================================================

-- 1) Columna de corte global (NULL = sin corte definido)
ALTER TABLE categorias ADD COLUMN IF NOT EXISTS clasificados INTEGER;

-- 2) Recalc grupo: excluir 'retirada' además de 'eliminado'
        ...
        BEGIN
            ...
            v_sql := format($sql$
                ...
                WITH equipos_grupo AS (
                    SELECT id FROM equipos WHERE grupo_id = %L AND estado NOT IN ('eliminado', 'retirada')
                ),
                partidos_grupo AS (
...
-- 3) Recalc general: excluir 'retirada' además de 'eliminado'
CREATE OR REPLACE FUNCTION fn_recalc_rankings_general(p_categoria_id UUID) RETURNS VOID AS $$
        BEGIN
            ...
            WITH equipos_cat AS (
                SELECT id FROM equipos WHERE categoria_id = p_categoria_id AND estado NOT IN ('eliminado', 'retirada')
            ),
            partidos_cat AS (
...
-- 4) Recalcular todo con las funciones nuevas
DO $$
DECLARE r RECORD;
BEGIN
...
def downgrade() -> None:
    op.execute("""
-- downgrade: quita la columna (las funciones quedan en su ultima version)
ALTER TABLE categorias DROP COLUMN IF EXISTS clasificados;
    """)
