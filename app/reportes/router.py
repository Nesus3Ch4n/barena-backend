...
async def fixture(torneo_id: str, categoria_id: str = None, user=Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
async def posiciones(torneo_id: str, user=Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
async def bracket(torneo_id: str, categoria_id: str = None, user=Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    if not categoria_id:
...
async def stats_equipo(torneo_id: str, equipo_id: str, user=Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
async def ranking_general(torneo_id: str, categoria_id: str = None, user=Depends(get_current_user), db: AsyncSession = Depends(get_db)):
