...
async def aprobar(equipo_id: str, body: EquipoAprobarIn, request: Request, user=Depends(require_roles("organizador", "super_admin")), db: AsyncSession = Depends(get_db)):
    ...
    equipo = await aprobar_equipo(db, equipo_id, body.grupo_id, body.seed, None, user.id, is_super, is_org)
...
@equipo_router.patch("/{equipo_id}", response_model=dict)
async def actualizar_equipo(equipo_id: str, body: EquipoUpdate, request: Request, user=Depends(require_roles("organizador", "super_admin")), db: AsyncSession = Depends(get_db)):
    is_super = "super_admin" in getattr(request.state, "roles", [])
    is_org = "organizador" in getattr(request.state, "roles", [])
