...
class Partido(Base):
    ...
    iniciado_en: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=True)
    iniciado_por: Mapped[str] = mapped_column(UUID(as_uuid=False), ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
    finalizado_en: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=True)
